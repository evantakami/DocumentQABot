...
def parse_bullets_with_header(text):
    ...
    text = text.strip()
    bullet_pattern = r'[○〇◯□]'
    if not re.search(bullet_pattern, text):
        return text, []
    match = re.search(bullet_pattern, text)
    header = text[:match.start()].strip()
    bullet_text = text[match.start():].strip()
    parts = re.split(r'(?=[○〇◯□])', bullet_text)
...
def row_to_markdown(row):
    ...
    imp_text = row['実施内容']
    conf_text = row['確認内容']
    imp_header, imp_list = parse_bullets_with_header(imp_text)
    conf_header, conf_list = parse_bullets_with_header(conf_text)
    imp_has_bullets = bool(imp_list)
    conf_has_bullets = bool(conf_list)
    md = []
    md.append(f"## 項番: {item_no}\n")
    md.append(f"**作業概要**: {summary}\n")
    md.append(f"**作業対象/環境等**: {environment}\n")
    if imp_has_bullets and conf_has_bullets:
        if imp_header:
            md.append(f"**実施内容（前置说明）**: {imp_header}\n")
        if conf_header:
            md.append(f"**確認内容（前置说明）**: {conf_header}\n")
        for i, (imp, conf) in enumerate(zip(imp_list, conf_list), start=1):
            md.append(f"{i}. **実施内容**: {imp}")
            md.append(f"   - **確認内容**: {conf}")
    else:
        md.append(f"**実施内容**: {imp_text.strip()}")
        md.append(f"**確認内容**: {conf_text.strip()}")
    return "\n".join(md)
